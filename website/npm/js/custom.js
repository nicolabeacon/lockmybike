...
$(document).ready(function () {
  ...
  mymap.on('locationfound', function (e) {
    ...
    if (
      (mymap.hasLayer(lyrLocate) && mymap.hasLayer(lyrFiltered)) ||
      mymap.hasLayer(lyrLocate) ||
      mymap.hasLayer(lyrFiltered)
    ) {
          ...
          onEachFeature: function (feature, layer) {
            ...
            $('#listOfData').append(
              "<li class='list-group-item' latitude=" +
                ...
                'at the address' +
                '&nbsp' +
                feature.properties.rack_street_address +
                '&nbsp' +
                feature.properties.zip +
                ' you will find' +
                '&nbsp' +
                feature.properties.qty +
                '&nbsp' +
                'spot/s' +
                '</li>'
            )
      ...
      $.getJSON('../bikedata.geojson', function (data) {
        ...
        $('#listOfData li').click(function (e) {
          const lat = $(this).attr('latitude')
          const lon = $(this).attr('longitude')
          const address = 'Address:' + '&nbsp' + $(this).attr('address')
          var popup = L.popup()
            .setLatLng([lat, lon])
            .setContent(address)
            .openOn(mymap)
